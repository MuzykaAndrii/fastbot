...
async def test_create_vocabulary(vocabulary_dal: VocabularySetDAL, session_mock_users: list[User]):
    owner = session_mock_users[0]
...
async def test_get_latest_user_vocabulary(session: AsyncSession, vocabulary_dal: VocabularySetDAL, session_mock_users: list[User]):
...
async def test_change_vocabulary_status(session: AsyncSession, vocabulary_dal: VocabularySetDAL, session_mock_users: list[User]):
    owner = session_mock_users[2]
    mock_vocabulary = {"owner_id": owner.id, "name": "Test Vocabulary"}
...
async def test_disable_user_active_vocabulary(
    ...
    session_mock_users: list[User]
):
    user = session_mock_users[0]
...
async def test_get_vocabulary_that_earliest_than_given(
    session: AsyncSession,
    vocabulary_dal: VocabularySetDAL,
    session_mock_users: list[User]
):
    ...
    earliest_vocabulary = await vocabulary_dal.get_vocabulary_that_earliest_than_given(user_id=user.id, vocabulary_id=mock_vocabulary_1["id"])
    assert earliest_vocabulary is not None
    assert earliest_vocabulary.name == mock_vocabulary_2["name"]